...
|---|---|---|---|---|---|---|---|
| **365** | **3** | **90** | **1,204** | **16.9%** | **0.760** | **0.772** | **+0.012** |
| 180 | 3 | 90 | 1,272 | 19.6% | 0.777 | 0.772 | −0.005 |
| 548 | 3 | 90 | 1,132 | 14.5% | 0.743 | 0.739 | −0.004 |
| 365 | 1 | 90 | 1,204 | 19.2% | 0.733 | 0.743 | +0.010 |
| 365 | 5 | 90 | 1,204 | 15.4% | 0.826 | 0.813 | −0.012 |
| 365 | 3 | 60 | 1,211 | 16.9% | 0.733 | 0.749 | +0.015 |
| 365 | 3 | 120 | 1,193 | 17.1% | 0.746 | 0.751 | +0.005 |

The returner rate stays in a 14–20% band and behavioural AUC in a 0.73–0.83
band across every variant, so neither is an artefact of one arbitrary
threshold. The acoustic delta ranges **−0.012 to +0.015**, changes sign three
times, and is negative in three of the seven. The conclusion is a property of
the data, not of the definitions.
...
training entirely — **176 of 1,380 here, 12.8% of the library**.
...
trained on that would learn to predict recency. The 176 held-out tracks then
...
history. Running the identical pipeline over three history lengths — an assumed
13.4 years, an age-derived 7.6, and the **10.4 years the account has actually
existed** (created 2016-02-05, per a Spotify export):

| | 13.4 years | 7.6 years | **10.4 years (real)** |
|---|---|---|---|
| Labelable tracks | 1,250 | 1,106 | **1,204** |
| Unlabelable (too new) | 124 (9%) | 265 (19%) | **176 (12.8%)** |
| Returner rate | 19.4% | 13.7% | **16.9%** |
| Hold-out interval width | 0.044 | 0.060 | **0.060** |

Two of those move for structural reasons worth separating. The **unlabelable
share** tracks history length directly, because a fixed 635-day cost is a
different fraction of each run. The **returner rate** moves with it because a
return needs somewhere to happen: a short history leaves less room after the
dormancy window for a track to come back in, so genuine returners get scored as
non-returners for want of runway. That is right-censoring, and it biases the
base rate down rather than adding noise.

The practical reading, and the reason the real date was worth chasing: nothing
in the model changed across those three columns. Getting the account's start
date right moved the headline more than any modelling decision in this project.
...
| Bounded, distinctive contexts encode better | general memory research | `enc_concentration` | **Rejected** — 70% attenuation under control |

### Why the bump does not rank tracks

The bump weight is a function of age at encoding. Across a long history it
varies from 0.35 to 0.99 and is genuinely informative. Across this one it
barely varies: the account opened at **age 8.7** and runs to 19, so **96.6% of
the library carries a weight above 0.85**. The 3.4% below it is listening from
before age 10 — the only stretch the literature expects to bind weakly, and
too small a slice to order anything by.

Which is the honest version of a fact that reads as a headline: *almost your
whole library is in the reminiscence bump* is simultaneously the most striking
thing the psychology says here and the reason it contributes nothing to the
ranking.
...
(**+0.16 log-odds per SD**) and reported separately.

`valence_intensity = |valence − 0.5|·2` is a *non-linear* transform of valence,
so that pair is not collinear and both stay in. It sits at **+0.21**, third
among acoustic terms behind cluster membership (**−0.49**) and tempo
(**+0.29**).
...
Those two clusters *do* separate on the outcome — **18.2% vs 8.9%** return
rate — and cluster membership is the strongest acoustic term in the model at
**−0.49**. That is a change from an earlier run of this project, and the reason
...
moved from **+0.000 to +0.019**. Changing history length then took it to
**+0.023** and then to **+0.012**, with the interval containing zero throughout
and its sign in the joint model flipping along the way.
...
computed on. This project's number is +0.012, CI [−0.019, +0.042], on a
two-pole library with 10.4 years of history, and it should not be lifted out of
that sentence.
...
at −1.00 log-odds** — ahead of every acoustic feature, ahead of play count and
completion, behind only skip rate.
...
forgettable", and the fact that it has ranged from −0.83 to −1.41 to −1.00
across history lengths is consistent with absorption rather than with a stable
effect.
...
strongest features. But 16.9% of tracks return while far more than 16.9% were
...
**The AUC is 0.76. Isn't that suspiciously good for a hard problem?**
...
The hold-out interval here is 0.060 wide around +0.012; roughly four times the
